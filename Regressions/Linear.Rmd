...
pacman::p_load(tidyverse, lmtest, estimatr, skimr, faraway)
```
...
#### Coding the model and extracting the residuals

Using the **lm** command we can execute a linear regression.
Than we assign to the original data set the model's residuals.

```{r, warning=FALSE, message=FALSE}
Model <- lm(DV ~ IV, data = newData)
summary(Model)
newData$resi <- Model$residuals
```


#### Ploting the dependent variable and the residuals and test the Pearson correlation

For detecting Heteroskedasticity by eye we can plot the scatter plot of the dependent variable and the residuals.
No relationship between the residuals and the dependent variables means Homoskedasticity.
```{r, warning=FALSE, message=FALSE}
cor.test(newData$DV, newData$resi)

newData %>% 
  ggplot(aes(x = DV, y = resi)) + 
  geom_point() +
  theme_classic() +
  geom_abline(slope = 0, color = 'red') + 
  xlab('Dependent variable') + ylab("Model's residuals")
```


#### Using the bptest command

Using the **bptest** command form the **lmtest** package.
Using the **ifelse** command we assign the strings **HC2** or **classical** whether there is a Heteroskedasticity or Homoskedasticity, respectively. 
```{r, warning=FALSE, message=FALSE}
...
```{r, warning=FALSE, message=FALSE}
Model <- lm_robust(DV ~ IV, se_type = regType, data = newData) # estimatr package
summary(Model)
```
...
Model1 <- lm_robust(DV ~ IV, se_type = regType, data = scalednewData) # estimatr package
summary(Model1)
...
Model1 <- lm_robust(DV ~ spin_habit_group + weekly_play_days + bet_habit_group + cz_group, se_type = regType, data = scalednewData) # estimatr package
summary(Model1)
